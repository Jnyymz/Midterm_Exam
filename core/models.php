...
function getBooksByAuthor($pdo, $authorID) {
	...
	$sql = "SELECT 
			...
			LEFT JOIN users ON books.userID = users.userID
...
function insertBook($pdo, $title, $genre, $price, $authorID, $dateAdded, $userID) {
    $sql = "INSERT INTO books (title, genre, price, authorID, dateAdded, userID) 
            VALUES (?,?,?,?,?,?)";
    $stmt = $pdo->prepare($sql);
    $executeQuery = $stmt->execute([$title, $genre, $price, $authorID, $dateAdded, $userID]);
